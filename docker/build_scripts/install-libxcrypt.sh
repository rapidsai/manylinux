...
if [ "$BASE_POLICY" == "musllinux" ]; then
	...
	exit 0
elif [ "${AUDITWHEEL_POLICY}" == "manylinux_2_27" ] || [ "${AUDITWHEEL_POLICY}" == "manylinux_2_28" ] || [ "${AUDITWHEEL_POLICY}" == "manylinux_2_31" ]; then
	echo "Skip libxcrypt installation on manylinux_{2_27,2_28,2_31}"
	exit 0
fi
